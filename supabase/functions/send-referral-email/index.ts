...
// FHIR Bundle Builder
const buildFhirBundle = (requestData: ReferralEmailRequest): any => {
  ...
  let clinicalFindings = `Důvod podezření: ${formData.suspicionReason || ''}`;
  
  if (formData.anamnesis) {
    clinicalFindings += `\n\nAnamnéza: ${formData.anamnesis}`;
  }
  
  if (formData.hasBloodThinners) {
    clinicalFindings += `\n\nAntikoagulancia: ${formData.bloodThinnersDetails || 'Ano'}`;
  }
  
  // Add imaging exam details
  if (imagingExams.length > 0) {
    clinicalFindings += '\n\nZobrazovací vyšetření:';
    imagingExams.forEach((exam: ImagingExam) => {
      const label = IMAGING_LABELS[exam.type] || exam.type;
      clinicalFindings += `\n- ${label} (${exam.date}): ${exam.description}`;
    });
  }
  
  if (formData.hasHistology && formData.histologyResult) {
    clinicalFindings += `\n\nHistologie (${formData.histologyDate || 'bez data'}): ${formData.histologyResult}`;
  }
  ...
  const bundle = {
    ...
    timestamp: timestamp,
    identifier: {
      system: "https://sarkom-fasttrack.example/form-id",
      value: formId
    },
    entry: [
      // Patient Resource
      {
        ...
        resource: {
          ...
          identifier: [
            {
              system: "https://sarkom-fasttrack.example/patient-id",
              value: formId, // Pseudonymized for GDPR
              type: {
                text: "Internal Form ID"
              }
            }
          ],
          name: [
            {
              text: `${requestData.patientContact.firstName} ${requestData.patientContact.lastName}`,
              family: requestData.patientContact.lastName,
              given: [requestData.patientContact.firstName]
            }
          ],
          telecom: [
            {
              system: "phone",
              value: requestData.patientContact.phone
            },
            ...(requestData.patientContact.email ? [{
              system: "email",
              value: requestData.patientContact.email
            }] : [])
          ],
          address: [
            {
              text: requestData.patientContact.address
            }
          ]
        }
      },
      ...
      {
        fullUrl: `urn:uuid:obs-clinical-findings`,
        resource: {
          resourceType: "Observation",
          id: "obs-clinical-findings",
          status: "final",
          code: {
            text: "Soft tissue mass characteristics and clinical findings"
          },
          subject: {
      ...
      {
        fullUrl: `urn:uuid:cond-suspected-sarcoma`,
        resource: {
          resourceType: "Condition",
          id: "cond-suspected-sarcoma",
          clinicalStatus: {
            coding: [{
      ...
      {
        fullUrl: `urn:uuid:service-request-referral`,
        resource: {
          resourceType: "ServiceRequest",
          id: "service-request-referral",
          status: "active",
          intent: "order",
          priority: "urgent",
          code: {
            text: "Referral to sarcoma center"
          },
          subject: {
            reference: `urn:uuid:${patientId}`
          },
          ...
          requester: {
            display: `${requestData.doctorContact.firstName} ${requestData.doctorContact.lastName}`,
            type: "Practitioner"
          },
          performer: [
            {
              display: DESTINATION_NAMES[requestData.destination] || requestData.destination,
              type: "Organization"
            }
          ],
          reasonReference: [
            {
              reference: `urn:uuid:cond-suspected-sarcoma`
            }
          ],
          note: [
            {
              text: `Referral destination: ${DESTINATION_NAMES[requestData.destination] || requestData.destination}`
            },
            ...(requestData.epacsShared ? [{
              text: "Medical images shared via ePACS"
            }] : [])
          ]
        }
      }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      const fhirBase64 = btoa(fhirJsonString); // Convert to base64
      
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19);
      const fhirFilename = `sarkom-fasttrack-referral-fhir-${timestamp}.json`;
      ...
      console.log(`FHIR Bundle generated successfully: ${fhirFilename}`);
    } catch (fhirError) {
      console.error("Error generating FHIR Bundle (email will still be sent):", fhirError);
